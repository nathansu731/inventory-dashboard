...
export const SavedReportsSearchBar = ({ searchTerm, setSearchTerm, selectedReports, handleBulkDownload, handleBulkDelete }: SavedReportsSearchBarProps) => {
    return (
        <div className="mb-2 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
            <div className="relative flex-1 max-w-sm">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4" />
                <Input
                    placeholder="Search reports by name, status, or model..."
                    value={searchTerm}
            ...
            {selectedReports.length > 0 && (
                <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={handleBulkDownload} className="flex items-center gap-2 bg-transparent">
