...
export async function POST() {
  ...
  const dueSourceIds = Object.values(sources)
    .filter(
      (source) =>
  ...
  const results: Array<{ sourceId: string; ok: boolean; status: "success" | "error"; message: string }> = []
  ...
  for (const sourceId of dueSourceIds) {
    ...
    try {
      ...
      appendDataSourceAudit({
        tenantRecord,
        type: result.ok ? "source_sync_due_success" : "source_sync_due_failed",
      ...
      try {
        ...
        // best effort; due sync continues even if notification write fails
      }
      results.push({
        sourceId,
        ok: result.ok,
