...
export async function GET(request: NextRequest) {
  ...
  if (!tableName || !region) {
    const missingConfig = NextResponse.redirect(new URL("/data-input?quickbooks=missing_config", request.url))
  ...
  if (!tenantRecord) {
    const tenantMissing = NextResponse.redirect(new URL("/data-input?quickbooks=tenant_not_found", request.url))
  ...
  if (role !== "admin") {
    const forbidden = NextResponse.redirect(new URL("/data-input?quickbooks=forbidden", request.url))
  ...
  if (!clientId) {
    const missingClient = NextResponse.redirect(new URL("/data-input?quickbooks=missing_client", request.url))
  ...
  response.cookies.set(QUICKBOOKS_STATE_COOKIE, statePayload, {
    httpOnly: true,
